...
class GTIReportConnector:
    ...
    def _build_markdown(self, attrs, name):
        ...
        if attrs.get("creation_date"):
            dt = datetime.datetime.fromtimestamp(int(attrs["creation_date"]), tz=datetime.timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
            lines.append(f"- **Published:** {dt}")
        if attrs.get("last_modification_date"):
            dt = datetime.datetime.fromtimestamp(int(attrs["last_modification_date"]), tz=datetime.timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    ...
    def ingest_report(self, report_data):
        ...
        published = datetime.datetime.fromtimestamp(int(creation_ts), tz=datetime.timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        ...
        while True:
            try:
                ...
                if last_run:
                    self.helper.log_info(
                        "[GTI] Last run: "
                        + datetime.datetime.fromtimestamp(last_run, tz=datetime.timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
                        + " UTC."
                    )
                else:
                ...
                if last_run is None or (timestamp - last_run) >= (interval_seconds - 60):
                    now_str = datetime.datetime.fromtimestamp(timestamp, tz=datetime.timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
                ...
                self.helper.log_error(f"Error during GTI run: {str(e)}")
