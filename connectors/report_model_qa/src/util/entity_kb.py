...
# Common suffixes appended to threat actor / intrusion set names in source
# reporting that do not appear in canonical KB entries. Stripping these before
# lookup allows "Handala Hack" to match canonical "Handala", "Lazarus Group"
# to match "Lazarus", etc. Order is longest-first to prevent partial strips
# (e.g. "APT Group" → strip "Group" → "APT", not "APT Gro").
_PERSONA_SUFFIXES: tuple = (
    " hacking team",
    " threat actor",
    " hacking group",
    " cyber group",
...
def _strip_persona_suffixes(name: str) -> str:
    """
    Strip known persona/group suffixes from a normalised entity name.
    Returns the stripped form, or the original if no suffix matched.
    Used as a fallback in KB lookup when the exact name is not found —
    allows variant names like "Handala Hack" to resolve to canonical
    "Handala", "Lazarus Group" to "Lazarus", etc.
    Suffixes are tried longest-first to prevent partial stripping.
    """
    for suffix in _PERSONA_SUFFIXES:
...
class EntityKB:
    ...
    def scan_text(self, text: str) -> List[Dict[str, Any]]:
        ...
        Also performs suffix-variant scanning: for each index term, constructs
        the set of known suffixes appended to that term and searches for those
        variants in the text. This allows "Handala Hack" in a document to match
        the canonical "Handala" KB entry even though "handala hack" is not in
        the index. Matches via suffix variant are recorded with the canonical
        name and entity_id but with matched_term set to the variant found.
        """
        ...
        # Build suffix variant search terms: for each indexed name, generate
        # all suffix-appended variants and map them back to the canonical entry.
        # Only generate variants for entity types that use persona suffixes.
        _SUFFIX_TYPES = frozenset({"Intrusion-Set", "Threat-Actor", "Malware", "Tool", "Campaign"})
        variant_index: Dict[str, List[tuple]] = {}  # variant → [(norm_name, entry)]
        for norm_name, entries in self._index.items():
            for entry in entries:
                if entry.entity_type not in _SUFFIX_TYPES:
                    continue
                for suffix in _PERSONA_SUFFIXES:
                    variant = norm_name + suffix
                    if len(variant) < _MIN_SCAN_NAME_LEN:
                        continue
                    if variant not in variant_index:
                        variant_index[variant] = []
                    variant_index[variant].append((norm_name, entry))

        # Primary scan: exact index terms
        for norm_name, entries in self._index.items():
            try:
        ...
        # Secondary scan: suffix variants that did not produce an exact match
        for variant, canon_list in variant_index.items():
            try:
            ...
            for norm_name, entry in canon_list:
                # Skip if the canonical entry was already matched by primary scan
                dedup_key = f"{entry.entity_id}:{norm_name}"
                if dedup_key in seen:
                    continue
                # Skip if this variant was already matched
                variant_key = f"{entry.entity_id}:{variant}"
                ...
                matches.append({
                    ...
                    "matched_term": variant,       # variant found in text
                    "entity_id": entry.entity_id,
                    "mitre_id": entry.mitre_id,
                    "snippet": snippet[:200],
                })
    ...
    def lookup(self, name: str) -> list:
        ...
        Falls back to suffix-stripped form if the exact normalised name is not
        in the index. This allows variant names used in source reporting
        ("Handala Hack", "Lazarus Group", "APT29 Team") to resolve to their
        canonical KB entries ("Handala", "Lazarus", "APT29") without requiring
        aliases to be manually maintained for every possible suffix variant.
        """
        norm = self._normalise(name)
        exact = self._index.get(norm)
        if exact:
            return exact
        stripped = _strip_persona_suffixes(norm)
        if stripped != norm:
            return self._index.get(stripped, [])
        return []
