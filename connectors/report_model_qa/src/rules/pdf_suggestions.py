...
def _smart_parse_malware_candidates(
    text: str,
    min_conf: float,
    max_candidates: int,
    kb: Optional[Any],
    ...
    """
    Extract named malware and tool entities from cue-phrase patterns that the
    actor extractor does not cover. Handles patterns like:

      "BEANSTALK Android backdoor"
      "the YEPCATCH dataminer"
      "a backdoor called DARKPULSAR"
      "ransomware known as LockBit"

    This catches purpose-built malicious software named in the document body
    without attribution cue phrases (tracked as / known as / attributed to).
    These phrases describe what the malware IS, not who operates it.
    """
    # Malware/tool type nouns. Ordered longest-first to avoid partial matches.
    _MALWARE_TYPES = (
        "remote access tool", "remote access trojan", "information stealer",
        "data miner", "dataminer", "backdoor", "ransomware", "infostealer",
        "keylogger", "downloader", "dropper", "loader", "wiper", "rootkit",
        "spyware", "botnet", "implant", "stager", "rat", "trojan", "malware",
        "exploit", "payload", "tool", "utility", "installer", "framework",
        "surveillance tool", "surveillanceware", "stalkerware",
    )
    _PLATFORM_QUALIFIERS = (
        "android", "windows", "linux", "macos", "ios", "mobile",
    )

    # Pattern A: [NAME] [optional platform] [malware_type]
    # e.g. "BEANSTALK Android backdoor", "YEPCATCH dataminer"
    _type_re = re.compile(
        r"(?P<name>[A-Z][A-Z0-9_\-]{2,30})"          # ALL_CAPS or Mixed name
        r"(?:\s+(?:" + "|".join(_PLATFORM_QUALIFIERS) + r"))?"  # optional platform
        r"\s+(?:" + "|".join(re.escape(t) for t in _MALWARE_TYPES) + r")",
        re.IGNORECASE,
    )

    # Pattern B: [malware_type] called/named/dubbed/known as [NAME]
    # e.g. "a backdoor called DARKPULSAR", "malware dubbed WannaCry"
    _called_re = re.compile(
        r"(?:" + "|".join(re.escape(t) for t in _MALWARE_TYPES) + r")"
        r"\s+(?:called|named|dubbed|tracked as|known as|designated)\s+"
        r"(?:the\s+)?(?P<name>[A-Z][A-Z0-9_\-]{2,30})",
        re.IGNORECASE,
    )

    # Pattern C: report title injection — parse the report name from the text
    # if it appears in the first 500 chars (title/header area of the PDF)
    # Already handled by Pattern A for title-embedded malware names.

    candidates: List[Dict[str, Any]] = []
    seen: set = set()

    def _process_match(name: str, pattern_id: str, span_start: int, span_end: int) -> None:
        name = name.strip()
        if not name or len(name) < 4:
            return
        if name.lower() in seen:
            return
        # Reject if it looks like a common English word (all lowercase after first char)
        if name[1:].islower() and len(name) < 7:
            return
        # Reject if it matches a stop word
        if name.lower() in {"the", "and", "for", "with", "this", "that", "from",
                            "into", "over", "under", "through", "after", "before"}:
            return

        seen.add(name.lower())
        occ = _count_occurrences(text, name)
        base_conf = 0.80 if name.isupper() or name[0].isupper() else 0.65
        conf = min(0.92, base_conf + (0.05 if occ >= 3 else 0) + (0.03 if occ >= 6 else 0))
        if conf < min_conf:
            return

        # KB reclassification: if the KB knows this name, use its authoritative type
        etype = "Malware"
        rationale = f"malware-cue pattern={pattern_id}"
        if kb is not None:
            entries = kb.lookup(name)
            if entries:
                etype = entries[0].entity_type
                rationale += f"; KB-confirmed as {etype}"

        evidence = _evidence_snippet(text, span_start, span_end)
        candidates.append({
            "name": name, "entity_type": etype, "confidence": conf,
            "rule": pattern_id, "rationale": rationale,
            "evidence": evidence, "occurrences": occ,
        })

    for m in _type_re.finditer(text):
        _process_match(m.group("name"), "MALWARE.TYPE_SUFFIX", m.start(), m.end())
        if len(candidates) >= max_candidates:
            break

    for m in _called_re.finditer(text):
        _process_match(m.group("name"), "MALWARE.CALLED_CUE", m.start(), m.end())
        if len(candidates) >= max_candidates:
            break
...
def _smart_parse_rows(
    ...
    # Malware/tool cue extraction — separate pass from actor cue extraction.
    # Finds named malware described with type nouns ("BEANSTALK backdoor",
    # "backdoor called DARKPULSAR") which actor cue phrases never match.
    malware_cands = _smart_parse_malware_candidates(
        text, min_conf=min_conf, max_candidates=max_cand, kb=kb
    )
    _log.info("Malware cue extractor: found %d candidates: %s",
              len(malware_cands),
              [(c["name"], c["entity_type"], c["confidence"]) for c in malware_cands[:10]])
    # Deduplicate: malware candidates that duplicate actor candidate names are dropped.
    actor_names = {c["name"].lower() for c in cands}
    malware_cands = [c for c in malware_cands if c["name"].lower() not in actor_names]
    all_cands = cands + malware_cands
    ...
    return rows, len(rows), all_cands
