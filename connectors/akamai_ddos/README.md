...
> **Status:** implemented, static-review remediation applied (F-01..F-10), **not yet
> deployed**. Before first live run, verify against a running OpenCTI 6.9.x: the
> `connect_id` helper attribute and `CaseIncident.generate_id` prefix, the exact
> `stix2` version resolved by `pycti==6.9.13`, and the `marking_definition`
> (`definition`) / `location` (`x_opencti_aliases`) filter keys. Recommended first
> run: Prolexic-only, one-day lookback (`AKAMAI_SIEM_ENABLED=false`,
> `AKAMAI_INITIAL_LOOKBACK_DAYS=1`) so the bundle is small enough to inspect by hand.

> **Known caveat (F-04):** one bundle is emitted per attack episode. A large SIEM
> episode (up to top-N source IPs times target systems) can exceed the platform bundle
> threshold and be silently truncated. A count guard (`AKAMAI_MAX_BUNDLE_OBJECTS`)
> logs a WARNING when this is possible; chunked container emission is not yet
> implemented and must be added before enabling SIEM at scale.
...
- Default marking **TLP:AMBER+STRICT**; times are source times. An unresolvable
  marking is a hard startup failure (unmarked first-party data is not emitted).
...
| `CONNECTOR_ID` | UUID for this connector instance (use a fresh UUIDv4) |
...
| `AKAMAI_MAX_BUNDLE_OBJECTS` | warn above this per-episode object count (default 2000; F-04 truncation guard) |
...
src/
  ...
  models.py        AttackEvent / SourceIp / cap_seen_ids (seen-id eviction)
...
tests/
  test_state.py    seen_attack_ids eviction regression (F-05)
```
